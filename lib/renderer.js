...
K.resourceLoader = function(fileName, callback) {
    ...
    var frontmatter = fileCache.extractFrontmatter(text);
...
var partial = module.exports.partial = function(name, locals, callback) {
    ...
    if (fileCache.isASyncHtml(fnamePartial)) {
    ...
    } else if (fileCache.isSyncHtml(fnamePartial)) {
...
var render = module.exports.render = function(options, fileName, data, done) {
    ...
    var doRenderAsync = function(entry, data, frontmatter, cb) {
        ...
        K(entry.fullpath, function(err, template) {
            if (err) done(err);
            else {
    ...
    var doRenderTemplate = function(template, data, callback) {
        ...
        if (fileCache.isSyncHtml(template)) {
        ...
        } else if (fileCache.isASyncHtml(template)) {
    ...
    if (fileCache.isSyncHtml(fileName)) {
    ...
    } else if (fileCache.isASyncHtml(fileName)) {
