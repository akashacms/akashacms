...
var renderFileAsync = module.exports.renderFileAsync = function(fileName, data, done) {
    ...
    data.oembed = function(url, template, callback) {
        util.log('oembed ' + url);
        oembed.fetch(url, { maxwidth: 6000 }, function(err, result) {
            if (err) {
                throw err;
            }
            util.log('retrieved oembed.. ' + util.inspect(result) + ' for url ' + url);
            partial(template, result, callback);
        });
    };
