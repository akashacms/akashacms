...
module.exports.createDocument = function(config, rootdir, docpath, metadata, content, done) {
	...
	var renderer = akasha.findRenderChain(docpath);
	if (!renderer) {
		renderer = rendererNull.match(docpath);
		renderer.renderSync = rendererNull.renderSync;
		renderer.render = rendererNull.render;
	}
    ...
    try {
		docEntry = {
			...
			renderedFileName: renderer.renderedFileName, // later: fname,
			fileExt: renderer.extension, // later: fnext,
			renderer: renderer,
			isdir: false,
			frontmatter: extractFrontmatter(text),
			data: text
