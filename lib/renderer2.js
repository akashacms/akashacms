...
var renderBody = function(fname, body, metadata, done) {
	// util.log('renderBody '+ util.inspect(fname));
	...
	if (fname.match(/\.md$/)) {
		...
		throw new Error('Use of Kernel templating '+ fnRender);
	} else {
		done(undefined, body);
	}
}
...
module.exports.render = function(akashacms, config, entryOrig, entryTemplate, metadata, done) {
    ...
    // util.log('render orig='+ entryOrig.path +' template='+ 
    //			(entryTemplate ? entryTemplate.path : ""));
