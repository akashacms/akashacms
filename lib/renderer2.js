...
var oembedRender = module.exports.oembedRender = function(arg, callback) {
    ...
    if (!("url" in arg) || typeof arg.url === "undefined")
        return callback(new Error("Must provide a URL to retrieve"));
 ...
 **/
var render = module.exports.render = function(akashacms, config, entryOrig, fnRender, metadata, body, done) {
    var entryRender;
    ...
    processExtension(akashacms, config, extensions, entryRender.frontmatter.text, metadata, function(err, newbody) {
        ...
        if (entryRender.frontmatter.layout) {
            return render(akashacms, config, entryOrig, entryRender.frontmatter.layout, metadata, newbody, done);
        } else {
