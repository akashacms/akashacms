...
module.exports.config = function(_akasha, config) {
  ...
  module.exports.render = render.bind(config);
};
...
function render(akashacms, entryOrig, entryTemplate, metadata, done) {
	...
	renderBody(fnRender, txtToRender, metadata, function(err, newbody) {
		...
		if (err) {
			...
			// mahaDuringRender
			// akasha.mahabhuta.process(newbody, metadata, _config.mahabhuta, function(err, rendered) {
			mahaRun(akashacms, _config, entryOrig, newbody, metadata, function(err, rendered) {
				...
				if (err) {
					...
					fileCache.readTemplate(entryOrig.frontmatter.yaml.layout, function(err, _entryTemplate) {
						...
						else module.exports.render(akashacms, entryOrig, _entryTemplate, metadata, done);
					});
